  ...
  handler: async (ctx, args) => {
    for (const storageId of args.storageIds) {
      const img = await ctx.db
        ...
        .first();
      if (img && !img.postId && !img.commentId && !img.aboutUserId && !img.isAvatar) {
        try {
...
export const cleanupOldPendingImages = internalMutation({
  args: {},
  handler: async (ctx) => {
    // Clean up images that are older than 24 hours and still in "pending" status (no postId, no commentId, no aboutUserId, and no isAvatar)
    const twentyFourHoursAgo = Date.now() - 24 * 60 * 60 * 1000;
    const allPending = await ctx.db.query('uploadedImages').collect();
    const oldPending = allPending.filter(
      (img) => !img.postId && !img.commentId && !img.aboutUserId && !img.isAvatar && img.createdAt < twentyFourHoursAgo,
    );
