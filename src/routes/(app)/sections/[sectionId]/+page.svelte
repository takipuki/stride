<script lang="ts">
  ...
  let activeTab = $state<'activities' | 'members'>('activities');
...
<div class="mx-auto flex w-full max-w-5xl flex-col gap-6 p-6 md:p-8">
  ...
  {:else}
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <!-- Left side: About Section & Navigation -->
      <div class="space-y-6 lg:col-span-2">
        ...
        <!-- Tabs Container -->
        <div class="space-y-4">
          <div class="flex gap-1 border-b border-border/40 pb-px">
            <button
              onclick={() => (activeTab = 'activities')}
              class="border-b-2 px-4 py-2 text-xs font-bold tracking-wider uppercase transition-all duration-200 {activeTab ===
              'activities'
                ? 'border-primary text-primary'
                : 'border-transparent text-muted-foreground hover:text-foreground'}"
            >
              Activities & Exams ({activities.length})
            </button>
            <button
              ...
              'members'
                ...
                : 'border-transparent text-muted-foreground hover:text-foreground'}"
            >
              Members ({teachers.length + students.length})
            </button>
          </div>
            ...
            <div class="space-y-4">
              ...
              {#if isTeacher}
                <div class="flex justify-end">
                  <Button
                    href="/activities/new?sectionId={section._id}"
                    size="sm"
              ...
              {:else}
                <div class="grid gap-3">
                  {#each activities as activity, index (activity._id)}
          ...
          {:else if activeTab === 'members'}
            <div class="space-y-6">
              <!-- Instructors Grid -->
              <div class="space-y-3">
                <h3 class="text-[11px] font-bold tracking-wider text-muted-foreground uppercase">Instructors</h3>
                ...
                {:else}
                  <div class="grid gap-3 sm:grid-cols-2">
                    {#each teachers as teacher (teacher._id)}
                      <div class="flex items-center gap-3 rounded-lg border border-border bg-card p-3 shadow-xs">
              ...
              <div class="space-y-3">
                <h3 class="text-[11px] font-bold tracking-wider text-muted-foreground uppercase">Students</h3>
      ...
      <div class="space-y-6">
          ...
          <Card.Content class="space-y-4">
            {#if teachers.length === 0}
              <div class="flex flex-col items-center justify-center p-4 text-center">
                ...
                <span class="mt-2 text-xs text-muted-foreground italic">No faculty assigned</span>
              </div>
            {:else}
              {#each teachers as teacher (teacher._id)}
                <div class="flex items-center gap-3">
        ...
        <Card.Root class="border-border bg-card shadow-xs">
          ...
          <Card.Content class="space-y-4">
            <div class="flex items-center justify-between border-b border-border/30 pb-3 text-xs">
              <span class="font-semibold text-muted-foreground">Total Students</span>
              <span class="font-bold text-foreground">{students.length}</span>
            </div>
            <div class="flex items-center justify-between border-b border-border/30 pb-3 text-xs">
              <span class="font-semibold text-muted-foreground">Active Activities</span>
              <span class="font-bold text-foreground">{activities.length}</span>
            </div>
            <div class="flex items-center justify-between text-xs">
              <span class="font-semibold text-muted-foreground">Created At</span>
              <span class="font-bold text-foreground">{formatDate(section.createdAt)}</span>
            </div>
          </Card.Content>
        </Card.Root>
      </div>
