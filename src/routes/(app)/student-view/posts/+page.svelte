...
<div class="flex h-full w-full overflow-hidden">
      ...
      <div class="mx-auto max-w-3xl space-y-6 p-4">
        ...
        {#each posts as post (post.id)}
          <Card.Root class="w-full">
            ...
            <Carousel.Root class="relative w-full overflow-hidden" opts={{ align: 'start', loop: false }}>
              <Carousel.Content class="-ms-0">
                ...
                {#each post.images as img}
                  <Carousel.Item class="basis-full shrink-0 grow-0 w-full">
              ...
              {#if post.images.length > 1}
                <Carousel.Previous class="absolute left-3 top-1/2 -translate-y-1/2 h-14 w-14 rounded-full border bg-black/70 text-white" />
                <Carousel.Next class="absolute right-3 top-1/2 -translate-y-1/2 h-14 w-14 rounded-full border bg-black/70 text-white" />
              {/if}
            </Carousel.Root>
            ...
            <!-- COMMENTS -->
            <div class="px-4 pt-2 text-xs font-semibold text-muted-foreground">
              Comments
            </div>
  ...
  <div class="w-80 border-l border-white/10 bg-muted/10 p-4">
    ...
    <div class="mb-4 text-sm font-semibold text-muted-foreground">
      Discover People
    </div>

    <div class="space-y-3">
      {#each users.filter(u => u.name.toLowerCase().includes(search.toLowerCase())) as user}
        <div
